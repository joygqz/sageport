...
- 主机地址、分组和备注；
- 凭据和密钥，包括私钥、密码、口令、AI API 密钥及同步服务商凭据。这些数据以明文保存在本地数据库文件中，不做额外加密；数据库文件的权限限制为仅当前用户可读写，但任何以你的身份运行的程序都能读取其内容。启用同步或导出备份时，这些数据会先用你设置的密码短语加密后再离开设备；
...
应用提供删除已存储记录的操作。不同操作系统和软件包管理器在卸载应用时，是否同时删除应用数据存在差异。如需彻底移除数据，用户应删除 Sageport 的应用数据目录；使用便携版时，该目录即 `Sageport.exe` 同级的 `data` 目录。
...
- 上传前，数据会先在您的设备上加密：应用使用 Argon2id 从同步口令派生密钥，再使用 AES-256-GCM 加密数据。发送给所选服务商的只有密文；为便于自动同步，连接期间同步口令会以明文保存在本地数据库中，但不会发送给存储服务商；
- 开发者无法访问或帮助找回同步口令。**如果本地数据库和另行保存的同步口令均丢失，之前同步的数据将无法解密或恢复；**
...
- 您的 API 密钥、对话内容以及经您逐次批准读取的终端或远程文件内容，会从您的设备直接发送到您配置的 AI 服务商，不经过开发者；
- 服务商如何存储、使用这些数据，或是否将其用于模型训练，完全受您所选 AI 服务商自身隐私政策约束，不受本政策约束；
- 默认情况下，AI 助手执行操作前需要您明确确认。终端输出、远程目录和文件内容在任何模式下都需要逐次批准；读取这些不可信内容后，后续操作会恢复人工审批。自主模式仅自动批准未受不可信内容影响且目标主机未要求人工审批的操作；
