...
Common shortcuts: <kbd>⌘N</kbd> new host, <kbd>⌘B</kbd> toggle side bar, <kbd>⌘J</kbd> toggle panel, <kbd>⌘L</kbd> toggle assistant, <kbd>⌘W</kbd> close tab, <kbd>⌘F</kbd> find in terminal, <kbd>⌘+</kbd>/<kbd>⌘−</kbd>/<kbd>⌘0</kbd> zoom the whole UI, <kbd>⌘,</kbd> settings. On Windows and Linux use <kbd>Ctrl</kbd>.

## Features

### Terminal

Sessions render through xterm.js with GPU acceleration, so output stays smooth under heavy load. Multiple sessions run side by side as tabs, with clickable links, full Unicode support and 10000 lines of scrollback. A VSCode style find bar (<kbd>⌘F</kbd>) searches the scrollback with match highlighting, and <kbd>⌘+</kbd>/<kbd>⌘−</kbd> zooms the whole workbench — terminal glyphs re-render at the native pixel size, so text stays crisp at every zoom level.
